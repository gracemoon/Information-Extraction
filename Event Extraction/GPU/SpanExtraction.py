...
test_tensorDataset=TensorDataset(torch.tensor(test_input_ids),
                    ...
                    torch.tensor(test_start_positions),
                    torch.tensor(test_end_positions))
...
for epoch in range(epochs):
    ...
    for step,batch in enumerate(train_dataLoader):
        batch_train_input_ids=batch[0].to(device)
        batch_train_attention_mask=batch[1].to(device)
        batch_train_start_position=batch[2].to(device)
        batch_train_end_position=batch[3].to(device)
        if(step==0):
        ...
        optimizer.step()
        if step%50==0:
            answers=extraction(batch_train_input_ids.detach().cpu().numpy(),start_logits.detach().cpu().numpy(),end_logits.detach().cpu().numpy())
            print(answers)
            print("%d"%loss.detach().cpu().numpy())
    model.eval()
    answers=[]
    for step,batch in enumerate(test_dataLoader):
        ...
        with torch.no_grad():
            ...
            step_answers=extraction(batch_train_input_ids.detach().cpu().numpy(),start_logits.detach().cpu().numpy(),end_logits.detach().cpu().numpy())
            answers=np.append(answers,step_answers)

    pred=pd.DataFrame(data=np.array([np.array(answers),test_labels]).T)
    pred.to_csv(str(step)+"_body.csv")
